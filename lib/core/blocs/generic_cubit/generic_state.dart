...
class GenericDimissLoadingState<T> extends GenericCubitState<T> {
  const GenericDimissLoadingState({required T data, required bool changed}) : super(data: data, changed: changed);
...
class GenericErrorState<T> extends GenericCubitState<T> {
  const GenericErrorState({required data, required Failure responseError, required bool changed})
